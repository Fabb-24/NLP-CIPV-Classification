...
class WindowDataLoader(Dataset):
    """
    A data loader for creating windows of conversation turns from a JSON dataset.
    """

    def __init__(self, json_dataset_path: str, tokenizer: PreTrainedTokenizer, test_size: float = 0.1, val_size: float = 0.2,
                 max_length: int = 512, random_state: int = 42, window_size: int = 3, remove_stopwords: bool = True,
                 lemmatize: bool = True, add_token_type_ids: bool = False):
        """
        Initializes the WindowDataLoader.
        ...
        Args:
            ...
            window_size (int): Number of turns to include in each window.
        ...
        print(f"[INFO] Initializing WindowDataLoader with:\n"
        ...
        # Create a dataset of windows of conversation turns
        self.wndw_dataset = None
        self.create_turn_windows_tokens(window_size=window_size)
        ...
        print(f"[INFO] WindowDataLoader initialized with {len(self.wndw_dataset)} windows.")
    ...
    def create_turn_windows_tokens(self, window_size: int = 3):
        """
        Creates windows of conversation turns from the dataset.
        ...
        windows = []
        loop = tqdm(conv_dataset, desc="[INFO] Creating windows of conversation turns", unit="sample")
        for sample in loop:
            windows.extend(conversation_to_windows(
                conversation=sample,
    ...
    def load_and_split_datasets(self):
        ...
        texts = [window[0] for window in self.wndw_dataset]
        raw_labels = [window[1] for window in self.wndw_dataset]
